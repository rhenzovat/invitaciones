...
export function descargar_reporte(params) {
  return from(apiClient.get(`${URL}/descargar_reporte_programacion`, { params })).pipe(
    map(result => result.data.result)
  ).toPromise();
}

export function listar_filtrar(params) {
  return from(apiClient.get(`${URL}/listar_filtrar`, { params })).pipe(
    map(result => result.data.result)
  ).toPromise();
}