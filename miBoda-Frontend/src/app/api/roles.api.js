...
export function agregar_permisos(params) {
  return from(apiClient.post(`${URL}/agregar_permisos`, params)).pipe(
    map((result) => result.data)
  ).toPromise();
}

export function listar_treeview(params) {
  return from(apiClient.get(`${URL}/listar_treeview`, { params })).pipe(
    map(result => result.data.result)
  ).toPromise();
}

export function copiar_permisos(params) {
  return from(apiClient.post(`${URL}/copiar_permisos`, params)).pipe(
    map((result) => result.data)
  ).toPromise();
}