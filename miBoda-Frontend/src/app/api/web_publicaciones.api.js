...
export const URL = '/web_publicaciones';
...
export function actualizar_seccion(params) {
  return from(apiFormDataClient.post(`${URL}/actualizar_seccion`, params)).pipe(
    map((r) => r.data.result)
  ).toPromise();
}

export function actualizar_paginas_banner(params) {
  return from(apiFormDataClient.post(`${URL}/actualizar_paginas_banner`, params)).pipe(
    map((r) => r.data.result)
  ).toPromise();
}
