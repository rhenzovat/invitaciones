-- DEPRECADO: el backend usa SeguridadMenuTreeviewService (ORM). No ejecutar en entornos nuevos.
-- Migración: 2026_05_21_130000_drop_datatreeview_and_treeview_sp.php
--
-- Mejora del SP: evita nodos huérfanos (objetos de módulos inactivos).
-- Solo incluye objetos de módulos activos (Activo='S').
-- Ejecutar si aparecen módulos como mod_13 en IdMenuPadre que no existen en el árbol.
...
BEGIN
  ...
  -- NIVEL 2 (bajo módulo): Objetos del módulo - SOLO de módulos activos (evita huérfanos)
  INSERT INTO datatreeview (IdMenu, IdMenuPadre, id_menu, id_menu_objetos, IdObjeto, Menu, Icon, Nivel, Orden, Activo, selected, Expanded, Tipo)
  SELECT
  ...
  LEFT JOIN sistema_objetos SO ON SO.id_objetos = SMO.id_objetos
  INNER JOIN sistema_modulo SM ON SM.id_modulo = SMO.id_modulo AND (SM.Activo = 'S' OR SM.Activo IS NULL)
  WHERE (SMO.Activo = 'S' OR SMO.Activo IS NULL);
